...
def log_likelihood_bernoulli(mu, target):
    ...
    *** note. ***
    ...
    ll_bernoulli = torch.sum(
        target * torch.log(mu + 1e-8) + (1 - target) * torch.log(1 - mu + 1e-8), dim=1
    )
...
def log_likelihood_normal(mu, logvar, z):
    ...
    *** note. ***
    ...
    ll_normal = torch.sum(
        -0.5 * (math.log(2 * math.pi) + logvar + (z - mu) ** 2 / torch.exp(logvar)),
        dim=1,
    )
...
def log_mean_exp(y):
    ...
    *** note. ***
    ...
    a, _ = torch.max(y, dim=1)
    exp_term = torch.exp(y - a.unsqueeze(1))
    mean_exp = torch.mean(exp_term, dim=1)
    lme = torch.log(mean_exp + 1e-8) + a
...
def kl_gaussian_gaussian_analytic(mu_q, logvar_q, mu_p, logvar_p):
    ...
    *** note. ***
    ...
    term = logvar_p - logvar_q + (var_q + (mu_q - mu_p) ** 2) / var_p - 1
    kl_gg = 0.5 * torch.sum(term, dim=1)
...
def kl_gaussian_gaussian_mc(mu_q, logvar_q, mu_p, logvar_p, num_samples=1):
    ...
    *** note. ***
    ...
    # reparameterization: sample z ~ q
    std_q = torch.exp(0.5 * logvar_q)
    eps = torch.randn_like(std_q)
    z = mu_q + std_q * eps

    # compute log q(z) and log p(z)
    # log_q = -0.5 * sum[ log(2*pi) + logvar_q + (z-mu_q)^2 / exp(logvar_q) ]
    log_q = torch.sum(
        -0.5
        * (math.log(2 * math.pi) + logvar_q + (z - mu_q) ** 2 / torch.exp(logvar_q)),
        dim=2,
    )
    # log_p = -0.5 * sum[ log(2*pi) + logvar_p + (z-mu_p)^2 / exp(logvar_p) ]
    log_p = torch.sum(
        -0.5
        * (math.log(2 * math.pi) + logvar_p + (z - mu_p) ** 2 / torch.exp(logvar_p)),
        dim=2,
    )

    # Monte Carlo estimate: E_q[log q - log p]
    kl_mc = torch.mean(log_q - log_p, dim=1)
