...
class Trainer:
    ...
    def train_epoch(self, dataloader, scaler):
        ...
        with tqdm(range(len(dataloader)), desc=f"Epoch : - lr: - Loss :") as progress:
            for x0, labels in dataloader:
                ...
                # Calculate the loss
                with autocast(
                    device_type=self.args.device, enabled=self.args.fp16_precision
                ):
    ...
    def sample(self, labels=None, cfg_scale=3.0, n_steps=None, set_seed=False):
        ...
        with torch.no_grad():
            ...
            if labels == None:
                labels = torch.randint(0, self.args.num_classes, (self.args.n_samples,), device=self.args.device)

            for i in range(n_steps):
                t_val = n_steps - i - 1
                t = torch.full((self.args.n_samples,), t_val, device=z.device, dtype=torch.long)
                lam = self.diffusion.get_lambda(t)
                lam_prev = self.diffusion.get_lambda(torch.clamp(t-1, min=0))

                # classifier-free guidance blend
                eps_uncond = self.eps_model(z)
                eps_cond   = self.eps_model(z, labels)
                eps = (1 + cfg_scale) * eps_cond - cfg_scale * eps_uncond

                # compute next clean estimate x_t
                alpha = self.diffusion.alpha_lambda(lam)
                sigma = self.diffusion.sigma_lambda(lam)
                x_t = (z - sigma * eps) / alpha

                # sample z_{t-1}
                z = self.diffusion.p_sample(z, lam, lam_prev, x_t)
