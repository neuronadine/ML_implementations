...
def get_loss_and_accuracy(logits, targets, eq_positions, mask, reduction='mean'):
    ...
    """
    B, S, V = logits.shape
    # 1) Log_probs of logits
    log_probs = F.log_softmax(logits, dim=-1)

    # 2) Gather log_probs of the RHS tokens
    gathered_log_probs = log_probs.gather(dim=-1, index=targets.unsqueeze(-1)).squeeze(-1)
    
    # 3) Build the mask for the RHS tokens
    positions = torch.arange(S, device=logits.device).unsqueeze(0)
    eq_positions = eq_positions.unsqueeze(1)
    after_eq = (positions > eq_positions)
    rhs_mask = after_eq & (mask == 1)

    masked_log_probs = gathered_log_probs * rhs_mask
    sum_log_probs = masked_log_probs.sum(dim=-1)
    count = rhs_mask.sum(dim=-1)

    per_sample_loss = - (sum_log_probs / (count + 1e-9)) # (B,)
    ...
    # 6) For the accuracy, we check if *all* RHS tokens match. 
    #    predicted token is argmax over V. shape -> (B, S)
    predictions = logits.argmax(dim=-1)

    # boolean of correct predictions for each position:
    correct_per_position = (predictions == targets)
    # But only positions in the RHS count, i.e. correct_per_position & rhs_mask
    # We want "1" if *every* RHS token was correct for that sample, else "0".
    # Let's do:
    all_correct_per_sample = []
    for i in range(B):
        # is every j in RHS correct?
        # That is, if there's ANY j in RHS with 'False', the sample is 0
        is_all_correct = (correct_per_position[i][rhs_mask[i]]).all()
        all_correct_per_sample.append(float(is_all_correct))

    all_correct_per_sample = torch.tensor(all_correct_per_sample, device=logits.device)  # shape (B,)

    # 7) Now reduce the accuracy as well
    if reduction == 'none':
        accuracy = all_correct_per_sample
    elif reduction == 'sum':
        accuracy = all_correct_per_sample.sum()
    elif reduction == 'mean':
        accuracy = all_correct_per_sample.mean()
    else:
