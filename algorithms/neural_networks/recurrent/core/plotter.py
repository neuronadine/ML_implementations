...
def plot_loss_accs(
    ...
    # Helper function to convert elements to a NumPy array on the CPU.
    def to_np(arr):
        return np.array([float(x.cpu().item() if isinstance(x, torch.Tensor) else x) for x in arr])
    ...
    if multiple_runs:
        all_steps = statistics["all_steps"]
        same_steps = all(len(steps) == len(all_steps[0]) for steps in all_steps)
        if same_steps:
            all_steps = to_np(all_steps[0]) + 1e-0  # Convert and add small offset
        else:
            all_steps = [to_np(steps) + 1e-0 for steps in all_steps]
            color_indices = np.linspace(0, 1, len(all_steps))
            colors = plt.cm.viridis(color_indices)
    else:
        all_steps = to_np(statistics["all_steps"]) + 1e-0
    ...
    for i, key in enumerate(["accuracy", "loss"]):
        ...
        if multiple_runs:
            ...
            if same_steps:
                zs_mean, zs_std = np.mean(zs, axis=0), np.std(zs, axis=0)
                ax.plot(all_steps, zs_mean, '-', color=color_1, label="Train", lw=linewidth)
                ax.fill_between(all_steps, zs_mean-zs_std, zs_mean+zs_std, color=color_1, alpha=0.2)
            else:  
                for j, z in enumerate(zs):
                    ax.plot(all_steps[j], to_np(z), '-', color=colors[j], label="Train", lw=linewidth/2)

            zs = np.array(statistics["test"][key])
            if same_steps:
                zs_mean, zs_std = np.mean(zs, axis=0), np.std(zs, axis=0)
                ax.plot(all_steps, zs_mean, '-', color=color_2, label="Eval", lw=linewidth)
                ax.fill_between(all_steps, zs_mean-zs_std, zs_mean+zs_std, color=color_2, alpha=0.2)
            else:  
                for j, z in enumerate(zs):
                    ax.plot(all_steps[j], to_np(z), '--', color=colors[j], label="Eval", lw=linewidth/2)

        else:
            ax.plot(all_steps, to_np(statistics["train"][key]), "-", color=color_1, label="Train", lw=linewidth)
            ax.plot(all_steps, to_np(statistics["test"][key]), "-", color=color_2, label="Eval", lw=linewidth)

        if log_x:
            ax.set_xscale('log')
        if log_y and key == "loss":
            ax.set_yscale('log')  # Only loss is set to log scale
        ax.tick_params(axis='y', labelsize='x-large')
        ax.tick_params(axis='x', labelsize='x-large')
        ax.set_xlabel("Training Steps (t)", fontsize=fontsize)
        s = "Accuracy" if key=="accuracy" else "Loss"
        ax.set_title(s, fontsize=fontsize)
        ax.grid(True)
        if multiple_runs and (not same_steps):
            legend_elements = [Line2D([0], [0], color='k', lw=linewidth, linestyle='-', label='Train'),
