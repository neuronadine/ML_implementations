    ...
    def forward(self, x, hx=None):
        ...
        for layer_idx, cell in enumerate(self.layers):
            ...
            for t in range(seq_len):
                x_t = output[:, t, :]  # (batch_size, input_size)
...
class LSTMLM(nn.Module):
    ...
    def forward(self, x, hidden_states=None):
        ...
        # LSTM layer
        output, (h, c) = self.lstm(embedded, hidden_states)  # (batch_size, sequence_length, hidden_size), (num_layers, batch_size, hidden_size)

        # MLP layer
        logits = self.classifier(output)  # (batch_size, sequence_length, vocabulary_size)
