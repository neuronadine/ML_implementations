...
class MultiHeadedAttention(nn.Module):
    ...
    def get_attention_weights(self, queries, keys):
        ...
        # This is a lower triangular matrix, where the upper triangle is set to -inf
        causal_mask = (
            torch.tril(
    ...
    def apply_attention(self, queries, keys, values):
        ...
        # Concatenate the attended values
    ...
    def split_heads(self, tensor):
        ...
        tensor = tensor.view(B, S, self.num_heads, self.head_size)
        return tensor.permute(0, 2, 1, 3)  # (batch_size, num_heads, sequence_length, head_size)
    ...
    def merge_heads(self, tensor):
        ...
        return tensor.contiguous().view(B, S, H * D)
    ...
    def forward(self, queries: Tensor, keys: Tensor, values: Tensor):
        ...
        context, attn_weights = self.apply_attention(Q_split, K_split, V_split)
...
class GPTEmbedding(nn.Module):
    ...
    def forward(self, tokens: Tensor) -> Tensor:
        ...
        seg_len = tokens.size(1)

        # Get the position embeddings
        pos_embed_slice = self.position_encoding[:seg_len, :]

        # Expand the position embeddings to match the batch size
        pos_embed_slice = pos_embed_slice.unsqueeze(0)

        # Add the token and position embeddings
        embeddings = token_embeddings + pos_embed_slice
        return embeddings
...
class GPT(nn.Module):
    ...
    def forward(
        ...
        logits = self.classifier(x)  # (batch_size, sequence_length, vocab_size)
