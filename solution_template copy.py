...
def find_bins(input_list, k):
    ...
    """
    input_list.extend([-1000, 1000])
    input_list.sort()
    h = int(len(input_list) / k)

    out = []

    for i in range(0, k + h + 1, h - 1):
        randint = np.around(
            np.random.uniform(input_list[i], input_list[i + 1]),
            decimals=2,
        )
        out.append(randint)
...
def heavyball_optimizer(x, inputs, alpha=0.9, beta=0.1):
    ...
    """
    X = np.zeros((inputs.shape[0]+2, inputs.shape[1]))
    X[1,:] = x
