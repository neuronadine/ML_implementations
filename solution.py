...
def ones_at_the_end(x):
    ...
    nums = arr[np.where(arr!=1)]
    return list(np.concatenate((nums, ones)))
...
def final_position(instructions):
    ...
    :return: int tuple
    """
    arr = np.array(instructions.split(" "))
    Dn = arr[np.where(arr=="down")].shape[0]
    Up = arr[np.where(arr=="up")].shape[0]
    R = arr[np.where(arr=="right")].shape[0]
    L = arr[np.where(arr=="left")].shape[0]
    return (R-L, Up-Dn)
...
def steps_to_one(i):
    ...
    def recurse(i, count):
        if i <= 1 :
            ...
            count += 1
            if i % 2 == 0:
                return recurse(i // 2, count)
            else :
                return recurse(3 * i + 1, count)
    return recurse(i, 0)
...
def find_bins(input_list, k):
    ...
    """
    input_list.sort()
    h = len(input_list) // k  # Number of items per bin

    out = [input_list[0] - 1]  # Starting edge slightly below the minimum value

    for i in range(1, k):
        low = input_list[i * h - 1]
        high = input_list[i * h]
        randint = np.around(
    ...
    out.append(input_list[-1] + 1)  # Ending edge slightly above the maximum value
...
def even_odd_ordered(X):
    ...
    """
    even = X[np.where(X%2==0)]
    odd = X[np.where(X%2!=0)]
...
def data_normalization(X):
    ...
    return np.hstack((((features - np.mean(features, axis=0))/np.std(features, axis=0)), target))
...
def entropy(p):
    ...
    """
    if int(np.sum(p)) == 1 and np.all(p >= 0):
...
def heavyball_optimizer(x, inputs, alpha=0.9, beta=0.1):
    ...
    """
    X = np.zeros((len(inputs)+2, x.shape[0]))
    X[1] = x

    for i in range(1, len(inputs)+1):
        X[i+1] = X[i]-(alpha*inputs[i-1])+(beta*(X[i]-X[i-1]))
...
class NearestCentroidClassifier:
    ...
    def predict(self, X):  # question B
        ...
        # return predictions
        pred_class = np.zeros(X.shape[0], dtype=int)
        for idx, point in enumerate(X):
            distances = np.array(
                [np.sum((point - self.centroids[k]) ** 2) for k in range(self.k)]
            )
            # Get the class of the minimum distance centroid
            pred_class[idx] = np.argmin(distances)
        return pred_class
